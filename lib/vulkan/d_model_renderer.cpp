...
namespace dal {
    ...
    void ActorSkinnedVK::apply_animation(const FrameInFlightIndex& index) {
        ...
        const auto size = std::min<uint32_t>(MAX_JOINT_SIZE, this->m_anim_state.getTransformArray().size());
        for (uint32_t i = 0; i < size; ++i)
            ubuf_data.m_transforms[i] = this->m_anim_state.getTransformArray()[i];
